		...
		<main className="repeating-bg">
			<Parallax image="/res/members/group/aces_2022.png">
				<div className="flex flex-col items-center pt-5 lg:pt-20 h-full text-white text-center">
					...
					<div className="p-8 lg:p-20 space-y-5 text-sm md:text-lg lg:text-3xl font-bold font-gold">
			...
			<div className="">
				...
				<section id="documents" className="flex justify-center mt-4">
					<a href="../res/documents/Engineering Society Constitution.pdf" target="_blank" className="border-2 border-mun-burgundy bg-white rounded-l-full text-black font-ops lg:text-3xl p-4">
						View Constitution
					</a>
					<a href="../res/documents/Society Executive Position Descriptions 2023.pdf" target="_blank" className="border-2 border-mun-burgundy bg-white rounded-r-full text-black font-ops lg:text-3xl p-4">
	...
	return (
		<div className="xl:flex xl:flex-col items-center">
			<div className="flex flex-center">
				<button className={`rounded-full p-1 border-2 bg-white border-mun-burgundy mt-2 font-extrabold font-ops transition-transform ${currentView === 1 ? "xl:scale-125 text-black" : "hidden"}`} onClick={() => setCurrentView(2)}>
					Show Past Societies
				</button>
				<button className={`rounded-full p-1 border-2 bg-white border-mun-burgundy mt-2 font-extrabold font-ops transition-transform ${currentView === 2 ? "xl:scale-125 text-black" : "hidden"}`} onClick={() => setCurrentView(1)}>
					Hide Past Societies
				</button>
			</div>
								...
								if (position === "members") {
									return (
										<div>
											...
											{name.map((item, index) => (
												<div className="text-white text-xl font-gold text-center" key={index}>
													{item}
												</div>
											))}
										</div>
...
function CurrentSociety({ society }) {
	return (
		<div id={society} className="space-y-4 lg:w-5/12 p-2">
			<div>
				<h1 className="text-black font-ops text-5xl bg-white border-t-4 border-l-4 border-r-4 rounded-t-3xl border-mun-burgundy text-center">{society.title}</h1>
				<h2 className="text-black font-ops text-3xl bg-white border-b-4 border-l-4 border-r-4 rounded-b-3xl border-mun-burgundy text-center">{society.years}</h2>
			</div>
